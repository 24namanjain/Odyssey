---
notion_page_id: 2dfff901-fc97-81cb-b4dd-f2ae861fe9b1
tags:
- kubernetes
- control-plane
- api-server
...
1. Pod information is stored in etcd without a nodeName property

1. Scheduler continuously monitors API server for unscheduled pods

1. When found, scheduler selects an appropriate worker node

1. Updates the pod's nodeName property to mark it as scheduled
...
1. **etcd**: The distributed database storing all cluster state and configuration

1. **Scheduler**: Assigns pods to nodes based on resource requirements and constraints

1. **Controller Manager**: Maintains desired state and manages various controllers

1. **Cloud Controller Manager**: Handles cloud-provider specific resources and integration
